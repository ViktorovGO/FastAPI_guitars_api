...
async def get_guitar_by_id(guitar: Guitar = Depends(guitar_by_id)):
    return guitar
...
async def partial_update_guitar(
    ...
    await crud.update_guitar(
        guitar_data=guitar_data, guitar=guitar, session=session, partial=True
    )
    return guitar
